...
@section('title_page')
    Create TheLoai - Admin - {{ config('app.name') }}
@endsection
@section('name_user')
    {{auth()->user()->Ten}}
@endsection
...
@section('content_card')
    ...
    <form action="{{route('admin.theloai.store')}}" method="post" class="form-control-sm">
        @csrf
        <div class="mb-10">
            <label for="exampleFormControlInput1" class="required form-label">Tên Thể Loại</label>
            <input name="TenTL" type="text" class="form-control form-control-solid"
                   placeholder="Nhập Tên Thể Loại" value="{{old('TenTL')}}" autocomplete="off">
        </div>
