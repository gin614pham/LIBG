...
@section('content_card')
    <form action="{{route('admin.vipham.store')}}" method="post" class="form-control-sm">
        @csrf
        <div class="mb-10">
            <label for="exampleFormControlInput1" class="required form-label">Độc Giả</label>
            <select class="form-select form-select-solid" data-control="select2"
                    data-placeholder="Chọn Độc Giả" data-select2-id="1" name="MaDG">
                <option value=""></option>
                @foreach($docGias as $item)
                    <option value="{{$item->MaDG}}">{{$item->MaDG."-".$item->Ten}}</option>
                    @endforeach
            </select>
        </div>
        <div class="mb-10">
            ...
            <input name="LyDoVP" type="text" class="form-control form-control-solid"
                   placeholder="Nhập Lý Do Vi Phạm" value="{{old('LyDoVP')}}" autocomplete="off">
        </div>
        <div class="mb-10">
            ...
            <input name="HinhThucXL" type="text" class="form-control form-control-solid"
                   placeholder="Nhập Hình Thức Xử Lý" value="{{old('HinhThucXL')}}" autocomplete="off">
        </div>
        <div class="mb-10">
